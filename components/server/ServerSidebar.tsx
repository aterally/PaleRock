    ...
    <aside style={styles.sidebar} onClick={() => setContextMenu(null)}>
      ...
      <div style={styles.channelList}>
            ...
            {uncategorized.map(ch => (
              <ChannelItem
                ...
                onContextMenu={canManageChannels ? (e) => {
                  e.preventDefault();
                  e.stopPropagation();
                  setContextMenu({ x: e.clientX, y: e.clientY, channelId: ch.id });
                } : undefined}
              />
            ))}
          </div>
        ...
        {byCategory.map(({ category, channels }) => (
          <CategorySection
            ...
            onContextMenu={(e, chId) => {
              e.preventDefault();
              e.stopPropagation();
              setContextMenu({ x: e.clientX, y: e.clientY, channelId: chId });
            }}
          />
        ))}
      ...
      {contextMenu && (
        <div style={{ ...styles.contextMenu, top: contextMenu.y, left: contextMenu.x }}>
          <button style={styles.contextItem} onClick={() => deleteChannel(contextMenu.channelId)}>
            <span style={{ color: 'var(--danger)' }}>Delete Channel</span>
          </button>
        </div>
      )}
      ...
      {showInviteModal && (
        <Modal title="INVITE PEOPLE" onClose={() => { setShowInviteModal(false); setInviteCode(''); }}>
          ...
          <div style={styles.inviteRow}>
            ...
            <button style={styles.copyBtn} onClick={() => navigator.clipboard.writeText(inviteCode)}>
              Copy
            </button>
          </div>
...
function CategorySection({ category, channels, activeChannelId, canManage, onChannelSelect, onContextMenu }: {
  ...
  return (
    <div style={styles.section}>
      <button style={styles.categoryHeader} onClick={() => setCollapsed(v => !v)}>
      ...
      {!collapsed && channels.map(ch => (
        <ChannelItem
          ...
          active={activeChannelId === ch.id}
          onClick={() => onChannelSelect(ch.id)}
          onContextMenu={canManage ? (e) => onContextMenu(e, ch.id) : undefined}
        />
      ))}
    </div>
  );
...
function ChannelItem({ channel, active, onClick, onContextMenu }: {
