...
function hasManageRoles(server: { ownerId: ObjectId; members: { userId: ObjectId; roles: string[] }[]; roles: { id: string; isDefault?: boolean; permissions: { manageRoles?: boolean; administrator?: boolean } }[] }, meId: ObjectId) {
  ...
  return server.roles.some(role => {
    ...
    return applies && (role.permissions?.manageRoles || role.permissions?.administrator);
  });
}
